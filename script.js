...
const firebaseConfig = {
  apiKey: "AIzaSyADyNBhCBqR0wMrmlEWcE7fHjgIK4ITF-o",
  authDomain: "water-brand-manager.firebaseapp.com",
  projectId: "water-brand-manager",
  storageBucket: "water-brand-manager.firebasestorage.app",
  messagingSenderId: "395758323906",
  appId: "1:395758323906:web:59773c34198593433f135b",
  measurementId: "G-MNHG4SET01"
};
...
let shops    = [];
let settings = {
  brand: 'AquaCMS', owner: '', wa: '', city: 'Karachi',
  bottles: [
    { size: '250ml', rate: 5  },
    { size: '500ml', rate: 10 },
    { size: '1L',    rate: 18 },
...
async function loadAllData() {
  ...
  try {
    ...
    toast('Error loading data. Check internet.', true);
  }
  showLoading(false);
  renderDashboard();
...
async function saveOrderDB(order, id) {
  const ref = id
    ? doc(db, 'orders', id)
...
async function saveShopDB(shop, id) {
  const ref = id
    ? doc(db, 'shops', id)
    : doc(collection(db, 'shops'));
  await setDoc(ref, { ...shop, id: ref.id });
  return ref.id;
}
...
async function deleteShopDB(id) {
...
async function saveShop() {
  ...
  try {
    if (shopEditIdx === -1) {
      const id = await saveShopDB(shop, null);
      shops.push({ ...shop, id });
      toast('Shop added ✅');
    } else {
      const existing = shops[shopEditIdx];
      await saveShopDB(shop, existing.id);
      shops[shopEditIdx] = { ...shop, id: existing.id };
      shopEditIdx = -1;
      toast('Shop updated ✅');
      cancelShopEdit();
    }
...
async function deleteShop(i) {
  if (!confirm('Delete this shop?')) return;
  try {
    await deleteShopDB(shops[i].id);
    shops.splice(i, 1);
    renderShops();
    toast('Shop deleted');
  } catch(e) {
    toast('Error deleting shop', true);
  }
}
...
async function saveOrder() {
  ...
  if (!shopName || !size || !qty || !rate) {
    toast('Please fill all fields', true); return;
  }
  ...
  try {
    if (orderEditIdx === -1) {
      const id = await saveOrderDB(order, null);
      orders.push({ ...order, id });
      toast('Order saved ✅');
    } else {
      const existing = orders[orderEditIdx];
      await saveOrderDB(order, existing.id);
      orders[orderEditIdx] = { ...order, id: existing.id };
      orderEditIdx = -1;
      toast('Order updated ✅');
      cancelOrderEdit();
    }
...
async function deleteOrder(i) {
  ...
  try {
    await deleteOrderDB(orders[i].id);
    orders.splice(i, 1);
    renderOrders();
    renderDashboard();
    toast('Order deleted');
  } catch(e) {
    toast('Error deleting order', true);
  }
}
...
function renderDashboard() {
  ...
  document.getElementById('top-pending-list').innerHTML = top.length
    ? top.map(([n,v]) =>
        `<div class="report-row"><span>${n}</span>
         <span class="amber" style="font-weight:600">Rs ${v.toLocaleString()}</span></div>`
      ).join('')
    : '<div style="color:var(--text3);font-size:13px;padding:12px 0">No pending amounts</div>';
...
function renderReports() {
  ...
  document.getElementById('r-shop-pending').innerHTML = spSorted.length
    ...
    : '<div style="color:var(--text3);font-size:13px;padding:12px 0">All cleared! No pending.</div>';

  const bottleMap = {};
  orders.forEach(o => { bottleMap[o.size] = (bottleMap[o.size]||0) + o.quantity; });
  ...
  document.getElementById('r-bottle-sizes').innerHTML = bSorted.length
    ...
    : '<div style="color:var(--text3);font-size:13px;padding:12px 0">No data yet</div>';
...
async function changePassword() {
  ...
  if (!current || !newPw || !confirm) { toast('Fill all password fields', true); return; }
  if (current !== saved)              { toast('Current password is wrong', true); return; }
  if (newPw.length < 6)              { toast('Min 6 characters required', true); return; }
